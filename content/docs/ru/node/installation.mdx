...
### Несколько нодов на одном сервере

Если вы хотите запускать несколько нодов на одном сервере, устанавливайте каждый нод с уникальным именем:

```bash
sudo bash -c "$(curl -sL https://github.com/PasarGuard/scripts/raw/main/pg-node.sh)" @ install --name node-eu-1
```

После установки используйте выбранное имя как префикс для команд управления:
...
Вы также можете использовать другие доступные подкоманды с тем же префиксом (`node-eu-1 ...`) для управления этим экземпляром нода.

Важно: не используйте одинаковые порты между нодами. Порты подключения и конфигурация Xray Core, связанная с каждым нодом, должны быть уникальными для каждого экземпляра.
