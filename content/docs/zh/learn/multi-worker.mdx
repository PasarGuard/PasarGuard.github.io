---
title: 多 Worker 模式
navigation: 多 Worker 模式
icon: Server
---

Multi-Worker 模式将 PasarGuard Panel 拆分为独立服务，使 API 流量与后台任务各自运行。这样可以提升吞吐量，并避免长时间任务阻塞面板。
...
- 节点/用户数量较多，需要更高的响应速度。
- 需要将定时任务与 API 流量隔离。
- 希望单独扩展 API worker 数量。

## 要求

- Worker 之间的协作需要 NATS。
- 所有服务必须共享同一数据库。
- 所有 worker 必须共享 `/var/lib/pasarguard`（模板、证书、运行时文件）。

<Callout type="warn">
Multi-Worker 模式要求 `NATS_ENABLED=True`。如果 NATS 未启用，node/scheduler worker 无法与 backend 协同。
</Callout>
...
<Callout type="info">
`ROLE` 在 `docker-compose.yml` 中按服务定义。使用下面的 Multi-Worker compose 时请不要在 `.env` 中设置它。
</Callout>
...
<Tabs items={['TimescaleDB (Recommended)', 'PostgreSQL', 'MySQL', 'MariaDB']}>
  <Tab value="TimescaleDB (Recommended)">
    **.env**
    ...
    SQLALCHEMY_DATABASE_URL = "postgresql+asyncpg://pasarguard:CHANGE_ME@127.0.0.1:6432/pasarguard"
    ```
  ...
  <Tab value="MySQL">
    **.env**
    ...
    SQLALCHEMY_DATABASE_URL = "mysql+asyncmy://pasarguard:CHANGE_ME@127.0.0.1:3306/pasarguard"
    ```
